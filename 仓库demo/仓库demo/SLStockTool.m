...
@implementation SLStockTool

+ (SLStockTool *)tool {
    static SLStockTool *tool = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        tool = [[SLStockTool alloc] init];
    });
    return tool;
}
...
-(BOOL)DataBase:(FMDatabase *)db insertKeyValues:(NSDictionary *)keyValues intoTable:(NSString *)tableName {
    if ([self isOpenDatabese:db]) {
...
-(BOOL)DataBase:(FMDatabase *)db updateTable:(NSString *)tableName setKeyValues:(NSDictionary *)keyValues {
    if ([self isOpenDatabese:db]) {
...
-(BOOL)DataBase:(FMDatabase *)db updateTable:(NSString *)tableName setKeyValues:(NSDictionary *)keyValues whereCondition:(NSDictionary *)condition {
    ...
    if ([self isOpenDatabese:db]) {
        for (NSString *key in keyValues) {
            ...
            NSLog(@"条件更新===>%@---%@---%@", sql, [keyValues valueForKey:key], [keyValues valueForKey:[condition allKeys][0]]);
...
-(NSArray *)DataBase:(FMDatabase *)db selectKeyTypes:(NSDictionary *)keyTypes fromTable:(NSString *)tableName whereCondition:(NSDictionary *)condition {
    if ([self isOpenDatabese:db]) {
        FMResultSet *result =  [db executeQuery:[NSString stringWithFormat:@"SELECT * FROM %@ WHERE %@ = ? LIMIT 10",tableName, [condition allKeys][0]], [condition allValues][0]];
...
-(void)clearDatabase:(FMDatabase *)db from:(NSString *)tableName {
    if ([self isOpenDatabese:db]) {
        BOOL flag = [db executeUpdate:[NSString stringWithFormat:@"DELETE FROM %@",tableName]];
        [self isFlag:flag];
    }
}
